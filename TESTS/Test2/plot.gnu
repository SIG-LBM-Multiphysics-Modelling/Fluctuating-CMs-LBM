...
# Reference slope -2 line through (1,-2) and (1.5,-3)
g(x) = -2*x+0.6
set xlabel '$\mathrm{log}_{10}(N_y)$' offset 0,0.2
set ylabel '$\mathrm{log}_{10}(\varepsilon)$' 
set key bottom left Left samplen 1
plot "error.txt" u (log10($1)):(log10($2)) w p lc 'blue' dt 2 pt 1 lw 3 title "Orthogonal",\
     "error.txt" u (log10($1)):(log10($2)) w p lc 'grey' dt 4 pt 2 lw 3 title "Non-orthogonal",\
 (x>=0.7 && x<=2.5 ? g(x) : 1/0) w l lw 2 lc 'black' notitle
