...
public:
    ...
    static void write(const std::string& filename, const int nx, const int ny, const int nz, 
        ViewType& u, ViewType& v, ViewType& w, const double U_ref)
    {
        std::ofstream output_file(filename);
        if (!output_file.is_open()) return;

        writeHeader(output_file, nx, ny, nz);
        writeCoordinates(output_file, nx, ny, nz);
        writePointData(output_file, nx, ny, nz, u, v, w, U_ref);
...
private:
    ...
    static void writeHeader(std::ofstream& output_file, int nx, int ny, int nz) {
    ...
    static void writeCoordinates(std::ofstream& output_file, int nx, int ny, int nz) {
        ...
        for (int k = 0; k < nz; ++k) output_file << k << ' ';
        output_file << '\n';
    }

    template <typename ViewType>
    static void writePointData(std::ofstream& output_file, const int nx, const int ny, const int nz, 
        ViewType& u, ViewType& v, ViewType& w, const double U_ref) {
