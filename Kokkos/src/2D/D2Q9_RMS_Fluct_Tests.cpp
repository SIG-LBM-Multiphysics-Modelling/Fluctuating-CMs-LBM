            ...
            const D2Q9 lattice, const Params parameters, RNGPool rng_pool)
{
    ...
    KOKKOS_LAMBDA(const int x, const int y) {
        ...
        double r1 = R*U + phi[1];
        double r2 = R*V + phi[2];
        double r3 = R*(U2+V2) + phi[3];
        r4 = parameters.omega1*r4 +  parameters.omega*R*(U2-V2) + phi[4];
        r5 = parameters.omega1*r5 +  parameters.omega*R*UV + phi[5];
        double r6 = R*U2*V + phi[6];
        double r7 = R*U*V2 + phi[7];
        double r8 = R*U2*V2 + phi[8];
...
static void case5_tau_sweep(){
    ...
    for(double tau_target : taus){
        ...
        const double psi = std::fabs(p.kBT/p.rho0-mean_u2)/(p.kBT/p.rho0)*100.;
        std::cout << std::setprecision(10)
                  << "tau=" << p.tau
                  << "  <u^2>=" << s.u2
                  << " <v^2>=" << s.v2
