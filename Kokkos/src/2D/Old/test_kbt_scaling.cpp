...
{
    std::string outdir="./TEST_kbt_scaling";
    for(int i=0;i<argc;i++){
        if(strcmp(argv[i], "-d")==0 && i+1<argc) outdir=argv[++i];
    }
    std::filesystem::create_directories(outdir);
    const std::string outpath = outdir + "/kbt_scaling_summary.txt";

    std::ofstream out(outpath, std::ios::trunc);
    out << "# kBT   target(kBT/rho0)   measured<u^2>   measured<v^2>\n";
    ...
    Kokkos::initialize(argc, argv);
    {
        ...
        const double kBT_list[3] = { 1.0/6000.0, 1.0/3000.0, 1.0/1500.0 };

        for(int c=0;c<3;c++){
            const Params p(rho0, kBT_list[c]);
